...
class Transformer_multilabel(Module):
    def __init__(self,
                 d_model: int,
                 d_input: int,
                 d_channel: int,
                 ...
                 pe: bool = False,
                 mask: bool = False):
        super(Transformer_multilabel, self).__init__()
        ...
        # 6 是视频的总时长，一秒做一个预测
        self.output_linear = torch.nn.Linear(d_model * d_input + d_model * d_channel, 6 * d_output)
    ...
    def forward(self, x, stage):
        ...
        output = output.view(-1, 6, self._d_output)
