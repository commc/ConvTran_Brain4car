...
# 选择要跑的模型
save_model_path = '/home/ubuntu/zsj/GTN-master/saved_model/brain4cars 85.9 batch=32.pkl'
...
path = '/home/ubuntu/zsj/GTN-master/dataset/annotations/validset/brain4cars_valid_dataset_random.json'
# 绘制HeatMap的命名准备工作
...
# 加载模型
net = torch.load(save_model_path, map_location=torch.device("cuda:0"))  # map_location 设置使用的设备，可能是因为原来的pkl是在colab上用GPU跑的
# 加载测试集数据
# test_dataset = MyDataset(path, 'test')
test_dataset = Brain4carDataset(path)
...
with torch.no_grad():
    ...
    for x, y in test_dataloader:
        ...
        # if label_index != y.data:
        #     print(x.cpu().numpy()[:, 100:, :])
        all_sample_X.append(encoding)
        all_sample_Y.append(y)
        if heatMap_or_not:
            for index, sample in enumerate(test_dataset.max_length_sample_inTest):
                if sample.cpu().numpy().tolist() in x.cpu().numpy().tolist():
                    target_index = x.cpu().numpy().tolist().index(sample.cpu().numpy().tolist())
                    print('正在绘制heatmap图...')
                    heatMap_all(score_input[target_index].cpu(), score_channel[target_index].cpu(), sample, 'heatmap_figure_in_test', file_name, ACCURACY, index)
                    print('heatmap图绘制完成！')
        if gather_or_not:
            for index, sample in enumerate(test_dataset.max_length_sample_inTest):
                if sample.cpu().numpy().tolist() in x.cpu().numpy().tolist():
                    target_index = x.cpu().numpy().tolist().index(sample.cpu().numpy().tolist())
                    print('正在绘制gather图...')
                    gather_by_tsne(gather_input[target_index].cpu().numpy(), np.arange(gather_input[target_index].shape[0]), index, file_name+' input_gather')
                    gather_by_tsne(gather_channel[target_index].cpu().numpy(), np.arange(gather_channel[target_index].shape[0]), index, file_name+' channel_gather')
                    print('gather图绘制完成！')
